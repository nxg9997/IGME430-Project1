...
const bf = (program, _input, _debug, _bits) => {
  ...
  const debug = (_debug === undefined) ? false : _debug;
  const bits = _bits || 8;

  // setup and initialization
  const maxVal = 2 ** bits - 1;
  let a = []; // data array
  let n = []; // nesting level pointer array
  let op = 0; // operation counter used in debug outputput
  let p = 0; // data pointer
  let x = 0; // x flag used when a loop is encountered and the assocated data pointer is already 0
  let output = '';
  ...
  function decrementPointer(i) {
    ...
    if (p > 0) {
      --p;
      if (debug) log(i, `array pos. now ${p}`);
    } else if (debug) log(i, `array pos. is already ${p}`);
  }
  ...
  function outputByte(i) {
    if (x) return;
    output += String.fromCharCode(a[p]);
